...
export default function LandingPage() {
  ...
  return <main>
    ...
    <section id="portfolio" className="section section-soft"><div className="container"><div className="center-heading scroll-reveal" data-scroll-reveal><span className="eyebrow">{content.portfolio.eyebrow}</span><h2>{content.portfolio.title}</h2><p className="muted">{content.portfolio.description}</p></div><div className="portfolio-grid">{content.portfolio.items.map((portfolio,index) => <article className={`portfolio-item scroll-reveal scroll-delay-${(index%3)+1}`} data-scroll-reveal key={`${portfolio.title}-${index}`}><EditableImage adminMode={adminMode} src={portfolio.image} alt={portfolio.title} target={{kind:"image",path:`portfolio.items.${index}.image`,label:`Portfolio ${index+1} image`,imageSection:"portfolio",imageIndex:index}} onEdit={edit}/><div className="portfolio-overlay"><span>{portfolio.category || "NyalaLagi"}</span><h3>{portfolio.title}</h3></div></article>)}</div></div></section>
