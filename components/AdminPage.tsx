...
import { Activity, BarChart3, ChevronDown, CircleDollarSign, ClipboardList, LogOut, Menu, Search, ShieldCheck, UserRound, Users, X, MapPin, Pencil, UserCog, UserPlus, Wrench, CheckCircle2 } from "lucide-react";
...
export default function AdminPage() {
  ...
  async function promoteToTechnician(user: Profile) {
    ...
    if (!window.confirm(`Jadikan ${user.name || user.email || "user ini"} sebagai teknisi?`)) return;
    try {
      ...
      setError(err?.message || "Gagal mengubah role menjadi teknisi.");
  ...
  async function assign(report: Profile, technicianId: string) {
    ...
    try {
      ...
      await updateDoc(doc(db, "reports", report.id), {
        ...
        technicianPhone: technician.phone_number || "",
        status: "ENGINEER_ASSIGNED",
        assignedAt: serverTimestamp(),
        updatedAt: serverTimestamp()
      });
    } catch (err: any) {
      setError(err?.message || "Gagal menugaskan teknisi.");
  ...
  return <div className="admin-shell">
    <aside className={`admin-sidebar ${mobileOpen ? "open" : ""}`}>
      ...
      <nav className="admin-menu">
        ...
        <button className={active === "technicians" ? "active" : ""} onClick={()=>{setActive("technicians");setMobileOpen(false)}}><UserCog size={18}/> Teknisi <span>{technicians.length}</span></button>
    ...
    <main className="admin-main">
      <header className="admin-topbar"><button className="admin-menu-toggle" onClick={()=>setMobileOpen(true)}><Menu size={22}/></button><div><span className="admin-kicker">ADMINISTRATOR</span><h1>{active === "dashboard" ? "Dashboard" : active === "customers" ? "Data Customer" : active === "reports" ? "Data Laporan" : "Manajemen Teknisi"}</h1></div><div className="admin-top-actions"><span className="admin-status"><i/> Sistem aktif</span><button className="admin-logout" onClick={logout}><LogOut size={16}/> Keluar</button></div></header>
      ...
      {active === "reports" && <section className="admin-panel admin-data-panel"><div className="admin-panel-title"><div><span>Reports collection</span><h3>Assignment & laporan customer</h3></div><div className="admin-report-count"><ClipboardList size={17}/> {reports.length} laporan</div></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Nomor</th><th>Customer</th><th>Masalah</th><th>Status</th><th>Teknisi</th><th>Lokasi</th></tr></thead><tbody>{reports.map(r=><tr key={r.id}><td><b>NYL-{String(r.id).slice(0,8).toUpperCase()}</b></td><td>{r.customerName || "—"}<small className="table-sub">{r.customerPhone || "—"}</small></td><td>{r.problemType || "—"}</td><td><span className="role-badge">{r.status || "—"}</span></td><td>{["COMPLETED","CANCELLED","REJECTED"].includes(r.status) ? (r.technicianName || "—") : <select className="input" value={r.technicianId || ""} disabled={assigning === r.id} onChange={e=>assign(r,e.target.value)} style={{minWidth:180,padding:"8px 10px"}}><option value="">{r.technicianId ? "Teknisi tersimpan" : "Pilih teknisi"}</option>{technicians.map(t=><option key={t.id} value={t.id}>{t.name || t.email || t.id}</option>)}</select>}{r.technicianName && <small className="table-sub">{r.technicianPhone || ""}</small>}</td><td>{r.location?.address || (r.location ? `${Number(r.location.latitude).toFixed(5)}, ${Number(r.location.longitude).toFixed(5)}` : "—")}</td></tr>)}</tbody></table>{reports.length === 0 && <div className="empty-admin">Belum ada laporan.</div>}</div></section>}

      {active === "technicians" && <section className="admin-panel admin-data-panel"><div className="admin-panel-title"><div><span>Technician management</span><h3>Teknisi aktif</h3></div><div className="admin-report-count"><Wrench size={17}/> {technicians.length} teknisi</div></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Teknisi</th><th>WhatsApp</th><th>Wilayah</th><th>Status</th></tr></thead><tbody>{technicians.map(t=><tr key={t.id}><td><div className="table-person"><div className="mini-avatar"><UserCog size={16}/></div><div><b>{t.name || "Tanpa nama"}</b><small>{t.email || "—"}</small></div></div></td><td>{t.phone_number || "—"}</td><td>{formatLocation(t)}</td><td><span className={`admin-badge ${t.status === "active" ? "success" : "muted-badge"}`}>{t.status || "—"}</span></td></tr>)}</tbody></table>{technicians.length === 0 && <div className="empty-admin">Belum ada teknisi. Anda dapat mengubah role customer menjadi teknisi dari daftar customer.</div>}</div><div style={{marginTop:18}}><h4 style={{margin:"0 0 10px"}}>Promosikan customer menjadi teknisi</h4><div style={{display:"grid",gap:8}}>{customers.filter(c=>c.status === "active").slice(0,20).map(c=><div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:"10px 12px",border:"1px solid var(--line,#e7eaf0)",borderRadius:12}}><span><b>{c.name || "Tanpa nama"}</b><small style={{display:"block",opacity:.65}}>{c.email || c.phone_number || "—"}</small></span><button className="btn btn-secondary" disabled={updatingRole === c.id} onClick={()=>promoteToTechnician(c)}>{updatingRole === c.id ? "Memproses..." : <><UserPlus size={15}/> Jadikan Teknisi</>}</button></div>)}</div></div></section>}
    </main>
  </div>;
}
